...
Function U_FENVREL()
	...
	while(cAlias)->(!EOF())
		...
		if ((STOD((cAlias)->ZZ1_B3VENC) - DATE())==(Val(StrTokArr(GETMV("FS_PODEENV"),",")[nX])));
				.And.((Val((cAlias)->ZZ1_EMAILE))<5);
				.And.(((STOD((cAlias)->ZZ1_ULTENV) - Date())*-1)==(Val(StrTokArr(GETMV("FS_PODEENV"),",")[nX]));
				.oR.Empty(STOD((cAlias)->ZZ1_ULTENV)))
			Aadd(aCliente,{;
				(cAlias)->ZZ1_CODPEN,;
				(cAlias)->ZZ1_CODFOR,;
				(cAlias)->ZZ1_NFORNE,;
			...
			cDiaVenc := StrTokArr(GETMV("FS_PODEENV"),",")[nX]
		endif
		IIF(nX <= 5,nX++,nX := 1)
		IIF(nX == 5,{(cAlias)->(DbSkip()), nX := 1},)
	End
	nX := 0
	...
	if(Len(aCliente) <> 0)
		...
		for nX := 1 to Len(aCliente)
			cTexto += "O Codigo de penhor: " + aCliente[nX][1] + " ira vencer em " + cDiaVenc +" dias."
			cTexto += "<br/>"
				...
				If nErro == 0
					...
					for nX := 1 to Len(aCliente)
						if(DBSeek(xFilial("ZZ1") + aCliente[nX][6] + aCliente[nX][2]))
							RecLock("ZZ1",.F.)
							ZZ1->ZZ1_EMAILE := cValToChar(Val(ZZ1->ZZ1_EMAILE) + 1)
							ZZ1->ZZ1_ULTENV := DATE()
							MSUNLOCK()
						endif
					next
